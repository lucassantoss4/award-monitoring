...
from core.utils.cleaner import formatar_data_br, limpar_resumo_tecnico
...
class Pipeline:
    ...
    # --- PROCESSAMENTO DE PREMIAÇÕES (Mantém formato BR) ---
    ...
    # --- PROCESSAMENTO DE EVENTOS (Novo Schema ISO YYYY-MM-DD) ---
    def _processar_eventos(self):
        ...
        try:
            ...
            for evento in eventos_raw:
                ...
                # 1. Aplica Curadoria Manual
                if nome_chave in curadoria:
                    logger.info(f"   ✨ Aplicando curadoria em evento: {nome_chave}")
                    evento.update(curadoria[nome_chave])

                # NOTA: Não formatamos a data aqui. O Dashboard espera YYYY-MM-DD (ISO)
                # O Schema Validator já garantiu que 'data_inicio' está correto.
